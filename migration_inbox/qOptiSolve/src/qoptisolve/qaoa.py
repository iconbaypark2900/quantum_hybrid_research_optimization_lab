...
try:  # Qiskit Algorithms as separate package (newer)
    ...
    try:
        ...
        # Last-resort lightweight fallbacks using SciPy/random search to preserve runtime
        try:
            from scipy.optimize import minimize  # type: ignore
        except Exception:  # pragma: no cover - if SciPy missing, provide a no-op minimize
        ...
        class SPSA:  # type: ignore
            ...
            def minimize(self, fun: Callable[[np.ndarray], float], x0: np.ndarray):
                # Very lightweight random search proxy for SPSA
                best_x = np.array(x0, dtype=float)
                best_val = float(fun(best_x))
                for i in range(self.maxiter):
...
class QAOA:
    ...
    def _add_portfolio_cost_hamiltonian(self, qc: QuantumCircuit, problem: 'PortfolioProblem', gamma: Parameter):
        ...
        # Add terms for each asset pair
        for i in range(n_assets):
            for j in range(n_assets):
                if i != j:
                    # Covariance term
                    scalar_coeff = float(problem.covariances[i, j]) / 2.0
                    # Only add significant terms based on the numeric coefficient
    ...
    def _bits_from_counts_key(self, key: str, num_qubits: int) -> np.ndarray:
        ...
        Qiskit may include spaces when multiple classical registers are present and
        returns bitstrings little-endian (rightmost bit corresponds to c[0]).
        This function removes spaces, pads if needed, reverses for qubit order,
        and returns an int numpy array of length num_qubits.
        """
        sanitized = key.replace(' ', '')
        if len(sanitized) < num_qubits:
            sanitized = sanitized.zfill(num_qubits)
        # Reverse to map rightmost classical bit to qubit 0 index
        ordered = sanitized[::-1]
    ...
    def solve_portfolio(self, problem: 'PortfolioProblem', p: int = 1, 
        ...
        # Evaluate objective for each measured bitstring and pick the best by cost
    ...
    def solve_maxcut(self, problem: 'MaxCutProblem', p: int = 1,
        ...
        # Evaluate cut value for each measured bitstring and pick the max
        ...
        return {
            ...
            'cut_value': best_cut,
            'optimal_params': result.x,
            'final_cost': -result.fun,  # keep optimization trace value for reference
            'convergence': result.nfev,
            'circuit': final_qc,
            'counts': final_counts
        }
